...
if test "$PHP_FIREBIRD" != "no"; then
  ...
  if test -x "$FB_CONFIG" && test "$PHP_FIREBIRD" = "yes"; then
    ...
    if test "$PHP_FIREBIRD" = "yes"; then
      FBIRD_INCDIR=/opt/firebird/include
      FBIRD_LIBDIR=/opt/firebird/lib
    else
      FBIRD_INCDIR=$PHP_FIREBIRD/include
      FBIRD_LIBDIR=$PHP_FIREBIRD/$PHP_LIBDIR
    fi

    PHP_CHECK_LIBRARY(fbclient, isc_detach_database,
    [
      FBIRD_LIBNAME=fbclient
    ], [
    ...
    PHP_ADD_LIBRARY_WITH_PATH($FBIRD_LIBNAME, $FBIRD_LIBDIR, FIREBIRD_SHARED_LIBADD)
    PHP_ADD_INCLUDE($FBIRD_INCDIR)
  fi
